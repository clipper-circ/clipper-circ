...
def login():
    ...
    plans = [(k, v, PLAN_PRICES[k], PLAN_DESCRIPTIONS.get(k, "")) for k, v in PLAN_LABELS.items()
             if k not in (PlanCode.COMPLIMENTARY, PlanCode.GIFT)]
        ...
        if sub:
            ...
            resend.Emails.send({
                ...
                "html": (
                    ...
                    f"<p>Please click the button below to access your Clipper Home Delivery account. "
                    f"This one-time link is valid for 7 days. "
                    f"Thanks for being a Duxbury Clipper Subscriber!</p>"
...
def confirm_hold():
    ...
    if hold_days < 7:
        ...
        return redirect(url_for("account") + "?tab=extras")
    if hold_days > 153:
        flash("Delivery holds cannot exceed 5 months (153 days). Please contact us if you need a longer hold.")
        return redirect(url_for("account") + "?tab=extras")
...
def _welcome_email_html(first_name, expiration, base_url, login_url=None):
    ...
    return (
        ...
        f"{login_btn}"
        f"<p>Questions? Call 781-934-2811 or email <a href='mailto:subscribe@duxburyclipper.com'>subscribe@duxburyclipper.com</a> . Please do not reply to this message — it is not monitored!</p>"
